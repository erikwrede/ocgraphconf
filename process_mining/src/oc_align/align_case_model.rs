...
impl ModelCaseChecker {
    ...
    pub fn bnb_v3_bfs(
        ...
        let mut open_list: BinaryHeap<OrderedSearchNode> = BinaryHeap::with_capacity(200_000);
        open_list.push(root_node.into());

        while let Some(ordered_node) = open_list.pop() {
            let node: SearchNode = ordered_node.into();
            counter += 1;
            ...
            if last_update.elapsed() >= Duration::from_secs(30) {
                ...
                println!(
                    "Current node unhit expected cost: {}",
                    node.unhit_expected_cost.iter().map(|c| c.expected_cost).sum::<usize>()
                );
            ...
            if node.marking.is_final_has_tokens() {
                ...
                //let alignment_mip = CaseAlignment::align_mip(query_case, &node.partial_case);
...

                {
                    ...
                    // sanity check, the cost of the reverse alignment should be the same as the forward alignmeent generated by align_mip

                    let reverse_cost = alignment.total_cost().unwrap();
                    if cost < node.static_min_cost {
                        panic!("COST VS STATIC MIN COST INVALID!!: {} vs {}", cost, node.static_min_cost);
                    }
                    if reverse_cost.ne(&cost) {
                        let real_reverse_cost = alignment.reverse().total_cost().unwrap();
                        // save an image of the alignment to the intermediates folder
                        // export_c2_with_alignment_image(
                        //     &node.partial_case,
                        //     &alignment.reverse(),
                        //     format!(
                        //         "/Users/erikwrede/dev/oss/process-mining/rust4pm/0thesis/errors/xxx{}.png",
                        //         counter
                        //     )
                        //     .as_str(),
                        //     Format::Png,
                        ...
                        // ).unwrap();
                        // 
                        // // also visualize the alignment generated by mip
                        // 
                        // export_c2_with_alignment_image(
                        //     &node.partial_case,
                        //     &alignment_mip,
                        //     format!(
                        //         "/Users/erikwrede/dev/oss/process-mining/rust4pm/0thesis/errors/xxx{}_mip.png",
                        //         counter
                        //     )
                        //     .as_str(),
                        //     Format::Png,
                        ...
                        // ).unwrap();
                        // 
                        // // also visualkize just the partial case without the alignment
                        // export_case_graph_image(
                        //     &node.partial_case,
                        //     format!(
                        //         "/Users/erikwrede/dev/oss/process-mining/rust4pm/0thesis/errors/xxx{}_partial_case.png",
                        //         counter
                        //     )
                        //     .as_str(),
                        //     Format::Png,
                        ...
                        panic!("MIP VS Reverse alignment cost mismatch: {} vs {}. Reversed reverse cost: {}. Counter {}", cost, reverse_cost, real_reverse_cost, counter);
                    }
                }
            ...
            // Push children into the BinaryHeap
        ...
        println!(
            "Total nodes explored (best-first branch-and-bound): {}",
            counter
        );
        println!("Maximum memory usage: {} MB", PEAK_ALLOC.peak_usage_as_mb());
        best_node
    }

    fn alignment_from_product_node<'a>(query_case: &'a CaseGraph, node: &'a SearchNode) -> CaseAlignment<'a> {
        let void_nodes: HashMap<usize, Node> = node
            .reverse_node_mapping
            .iter()
            .filter_map(|(id, mapping)| {
    ...
    fn calculate_min_expected_cost(
        ...
        fn projected_cost(tf: &bool) -> usize {
            ...
            if (*tf) {
                ...
                //FIXME !!!! fixme wichtig für impl ! es sind immer nur die edges!! nodes können wir nicht vorab abschätzen, auch dann nicht wenn die node laut model nur ein einziges E2O hat, weil die node für ein ganz anderes objekt verwendet werden kann!!
                // verify after thesis for now use 1 to be safe
                return 2;
            } else {
        ...
        node.open_query_nodes
            ...
            .for_each(|open_node| {
                ...
                let void_cost = query_case
                    ...
                    .count();
                // fixme: we disable more agresssive void cost as it's currently not described in the 0thesis
                // and might still have errors
                let void_cost = 0;
                ...
                if (node.min_cost > new_min_cost + min_remaining_cost as f64) {
                    ...
                    println!("previous share of heuristic {}", node.min_cost - node.static_min_cost);
